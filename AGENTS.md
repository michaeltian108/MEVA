...
This file is the sole active bootstrap and operating manual. Read it completely
once at the start of a fresh session, then load and validate the active state
before the first project action. The former `Agent.md` filename is stale and
must not be treated as a second source of instructions.
...
The automatically activated primary agent is the primary MEVA Orchestrator. It
...
It is not a spawnable custom role or worker. The only spawnable workers are:
...
- **F7 activation:** the primary contract is automatically loaded through
  `AGENTS.md`; no custom orchestrator role exists. Only
...
Ship `AGENTS.md`, `.codex/config.toml`, exactly five worker TOMLs,
